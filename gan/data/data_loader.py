...
class AudioDataset(Dataset):
    def __init__(self, clean_path, noisy_path, standardize=None,
        ...
        self.data = []
        clean_files = [file for file in os.listdir(clean_path) if file.endswith('.pt')]
        noisy_files = [file for file in os.listdir(noisy_path) if file.endswith('.pt')]
        
        for clean_file, noisy_file in tqdm(zip(clean_files, noisy_files), desc="Loading data"):
            clean_stft = torch.load(os.path.join(clean_path, clean_file))
            noisy_stft = torch.load(os.path.join(noisy_path, noisy_file))
            self.data.append((clean_stft, noisy_stft))
