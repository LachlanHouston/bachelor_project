...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if torch.cuda.is_available():
            ...
            return clean_stft.squeeze().to(device), noisy_stft.squeeze().to(device)
...
class AudioDataset_split(Dataset):
    def __init__(self, clean_path, noisy_path, is_train, fraction=1.0):
        ...
        self.clean_path = clean_path
        self.noisy_path = noisy_path
        # If train, sample a fraction of the data
        # If val, take the other fraction of the data
        if is_train:
            num_files = int(fraction * len(os.listdir(clean_path)))
            self.clean_files = sorted([file for file in os.listdir(clean_path) if file.endswith('.wav')])[:num_files]
            self.noisy_files = sorted([file for file in os.listdir(noisy_path) if file.endswith('.wav')])[:num_files]
        else:
            num_files = int((1-fraction) * len(os.listdir(clean_path)))
            self.clean_files = sorted([file for file in os.listdir(clean_path) if file.endswith('.wav')])[-num_files:]
            self.noisy_files = sorted([file for file in os.listdir(noisy_path) if file.endswith('.wav')])[-num_files:]
...
class AudioDataModule_train_split(L.LightningDataModule):
    def __init__(self, clean_path, noisy_path, batch_size=16, num_workers=16, fraction=1.0):
        super(AudioDataModule_train_split, self).__init__()
        self.clean_path = clean_path
        self.noisy_path = noisy_path
        self.batch_size = batch_size
        self.num_workers = num_workers if torch.cuda.is_available() else 1
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_dataset = AudioDataset(self.clean_path, self.noisy_path, is_train=True, fraction=self.fraction)
            self.val_dataset = AudioDataset(self.clean_path, self.noisy_path, is_train=False, fraction=(1-self.fraction))
