    ...
    def on_train_epoch_end(self):
        ...
        if self.current_epoch % 1 == 0:
            for name, param in self.generator.named_parameters():
                self.log(f'Norms/Gen_{name}_norm', param.norm(), on_step=False, on_epoch=True, prog_bar=False, logger=True)
            for name, param in self.discriminator.named_parameters():
                self.log(f'Norms/Disc_{name}_norm', param.norm(), on_step=False, on_epoch=True, prog_bar=False, logger=True)
            # Also log the overall norm of the generator and discriminator parameters
            self.log('Norms/Gen_mean_norm', torch.norm(torch.cat([param.view(-1) for param in self.generator.parameters()])), on_step=False, on_epoch=True, prog_bar=False, logger=True)
            self.log('Norms/Disc_mean_norm', torch.norm(torch.cat([param.view(-1) for param in self.discriminator.parameters()])), on_step=False, on_epoch=True, prog_bar=False, logger=True)
